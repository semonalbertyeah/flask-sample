...
import os, sys, socket, struct, select, time, warnings
import random
import threading
...
def do_one_ping(dest_addr, timeout):
    try:
        ...
        if e.errno == 101:
            ...
            warnings.warn("Ping %r, but network is unreachable." % dest_addr)
            delay = None
        else:
            raise
